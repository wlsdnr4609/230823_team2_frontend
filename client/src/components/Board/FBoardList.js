...
class FBoardList extends Component {
    ...
    callSwToolListApi = async (page = 1) => {
        try {
            const response = await axios.get(`/api/list?btype=F&page=${page}&perPageNum=10`);
    ...
    renderTableRows = () => {
        ...
        return currentPageData.map((data, index) => (
            <tr key={data.bid}>
                ...
                <td>
                    <Link to={`FContentView/${data.bid}`}>{data.title}</Link>
                </td>
                <td>{data.niname}</td>
                <td>{data.counts}</td>
                ...
                <article className="s_cnt mp_pro_li ct1 mp_pro_li_admin">
                    <div className="li_top">
                        ...
                        <td class="fileBox fileBox_w1">
                            ...
                            <Link to="/FBoardView/" className="sch_bt2 wi_au">
                                글쓰기
                            </Link>
                        </div>
                    </div>
