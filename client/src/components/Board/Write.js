...
class Write extends Component {
    ...
    //     await axios.post('/api/member/readMember', {
    ...
    callSwToolInfoApi = async () => {
        try {
            const response = await axios.post('/api/member/readMember', {
                email: this.state.email,
            });
            this.setState({ mid: data.mid });
    ...
    submitClick = async (type, e) => {
        ...
        this.btype_checker = $('#btype_val').val();


        this.fnValidate = (e) => {
            if (this.btype_checker === '') {
                $('#btype_val').addClass('border_validate_err');
