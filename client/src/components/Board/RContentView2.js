...
class RContentView2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
        ...
        axios.post('/api/read', {
            ...
            .then(response => {
                try {
                    ...
                    this.setState({ niname2: data.niname })
    ...
    handleFileInput(type, e) {
        if (type == 'file') {
            $('#imagefile').val(e.target.files.name)
        } else if (type == 'file2') {
            $('#imagefile2').val(e.target.files.name)
        } else if (type == 'manual') {
            $('#manualfile').val(e.target.files.name)
        }
        this.setState({
    ...
    handlePostMenual() {
        ...
        return axios.post("/api/uploadAjax", formData).then(res => {
            ...
            alert('5. 작업중 오류가 발생하였습니다.', error, 'error', '닫기')
        })
    }
                ...
                <article class="s_cnt mp_pro_li ct1">
                    ...
                    <div class="bo_w re1_wrap re1_wrap_writer">
                        <form name="frm" id="frm" action="" onsubmit="" method="post" >
                            <input id="mid" type="hidden" name="mid" value={this.state.mid} />
                            <input id="is_Email" type="hidden" value="guest" />
                            <input id="bid" type="hidden" name="bid" value={this.state.before_swtcode} />
                            <article class="res_w">
                                ...
                                <div class="tb_outline">
                                    <table class="table_ty1">
                                        <tr>
                                            ...
                                            <td>
                                                <dev name="niname" id="is_Swt_niname" class="">{this.state.niname2}</dev>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>
                                    ...
                                    <div class="btn_confirm mt20" style={{ "margin-bottom": "44px" }}>
                                        <Link to={'/RboardList'} className="bt_ty bt_ty1 cancel_ty1">취소</Link>
